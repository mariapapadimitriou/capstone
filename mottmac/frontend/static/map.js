...
function updateLegend() {
  ...
  if (draw.getAll().features.length >= 1) {
    for (let i = 0; i < draw.getAll().features.length; i++) {
      ...
      routes.push("<div class='row' style='display: flex; flex-direction: row; width: 100%; justify-content: space-between;'><span style='margin-left: 10px; color:")
      routes.push(id_colours[routeid])
      routes.push(";'>")
      routes.push("<b>Route")
      routes.push(i+1)
      routes.push("</b>&nbsp;&nbsp;&nbsp;" + roundToTwo(turf.length(id_coords[routeid])) + "km</span><span><button class='saveroutebtn' id='save" + i + "'onclick='saveRoute(this.id)'><i class='fas fa-save'></i></button></span></div>")
